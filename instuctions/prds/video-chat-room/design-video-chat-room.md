...
**Модель установления соединения (mesh):** при входе нового участника для каждой новой пары устанавливается P2P-соединение по **детерминированному правилу**: offer инициирует тот участник пары, чей `socketId` лексикографически **меньше** — независимо от того, кто из них новичок, а кто уже был в комнате. Это исключает glare/двойные offer и снимает гонку при одновременном входе.
...
sequenceDiagram
    ...
    participant Ex as Другой участник (A)
    ...
    alt комната полна
        ...
        Note over New,Ex: initiator пары = peer с меньшим socketId<br/>(в примере меньший socketId у A)
...
**Правило initiator (анти-glare):** для каждой пары участников offer инициирует тот, чей `socketId` лексикографически **меньше** — независимо от того, кто из пары новичок, а кто уже был в комнате. Это даёт детерминированную роль без «вежливого peer» и предотвращает двойной offer при одновременном входе (US-5 race). В sequence-диаграмме выше показан частный случай, когда меньший `socketId` оказался у уже присутствовавшего участника A; если бы меньший `socketId` был у новичка C, offer инициировал бы он.
...
### 7.3 Тумблер камеры (критичный нюанс — п. 19 PRD / US-7)
...
> **Нюанс семантики mute (US-7).** При `enabled = false` дорожка технически не останавливается — peer продолжает получать поток, но в виде «тишины»/пустых кадров, поэтому удалённый участник звук не слышит. Это стандартная и приемлемая трактовка требования US-7 «аудио перестаёт передаваться остальным»; полноценная остановка дорожки (`stop()`), как для камеры (п. 19 PRD), для микрофона не требуется.
...
| Имя >30 / спецсимволы | п. 38 PRD | Обрезка до 30, фильтрация спецсимволов, экранирование; дублируется на сервере |
...
| XSS в имени/сообщении | п. 39 PRD | Экранирование при рендере (React по умолчанию + sanitize на сервере) |
...
| **Тумблер камеры не гасит лампочку** | Нарушение п. 19 PRD | Обязательный `track.stop()` + `replaceTrack` (§7.3), а не `enabled=false` |
