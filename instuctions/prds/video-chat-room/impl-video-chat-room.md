...
- [x] 2. Локальный HTTPS / secure context
  ...
  - _Реализация: `scripts/generate-cert.sh`, `npm run certs`, авто-HTTPS в `server/src/config.js` и `client/vite.config.js`; prod — один origin на `:3001`._

---

## Блок B. Сервер: комнаты, лимит, жизненный цикл

- [x] 3. `RoomRegistry` — in-memory модель комнат (`rooms.js`)
  ...
  - _Тесты: `server/test/rooms.test.js`._

- [x] 4. Валидация (`validation.js`)
  ...
  - _Тесты: `server/test/validation.test.js`._

- [x] 5. Обработчик `room:join` + события состава
  ...
  - _Реализация: `server/src/roomHandlers.js`._

- [x] 6. Обработчики `room:leave` / `disconnect`
...
- [x] 7. Relay сигналинга (`signaling.js`)
...
- [x] 8. Чат и системные события (`chat.js`)
...
- [x] 10. `PeerConnectionManager` — набор `RTCPeerConnection` (`webrtc/PeerConnectionManager.js`)
  ...
  - _Решение TDD §14 TBD-4: нативный `RTCPeerConnection`. Renegotiation при позднем включении камеры (US-6)._
  - _Тесты: `client/src/webrtc/PeerConnectionManager.test.js`._

- [x] 11. `useSignaling` — socket.io-client (`socket/useSignaling.js`)
...
- [x] 12. Тумблеры микрофона и камеры
...
- [x] 13. `StartScreen` — стартовый экран
  ...
  - _Решение TDD §14 TBD-5: `nanoid(8)` для `roomId`._

- [x] 14. Вход по ссылке-приглашению + копирование ссылки
  ...
  - _Реализация: `RoomScreen` + `Controls.onCopyLink`, `EntryScreen` с превью устройств._

- [x] 15. `VideoGrid` + `VideoTile` — видеосетка
...
- [x] 16. `ChatPanel` + `ParticipantList`
  ...
  - _Реализация: `ChatPanel.jsx`, `ParticipantList.jsx` в боковой панели `RoomScreen`._

- [x] 17. `Controls` + выход из комнаты
  ...
  - _Дополнительно: выбор устройств ввода/вывода (`DeviceMenu`), индикатор «говорит» (`useSpeaking`)._

- [x] 18. `RoomScreen` — оркестрация
...
- [x] 19. Экраны/баннеры ошибок окружения
  ...
  - _Реализация: `NoticeScreen`, `Toast`, `lib/support.js`, баннер autoplay в `RoomScreen`._

- [x] 20. Деградация без падения приложения
  ...
  - _Реализация: индикатор «Соединение не установлено» на плитке при `connectionState === 'failed'`; ICE-restart не выполняется (TDD §14 TBD-3)._

---

## Блок G. Тестирование

- [x] 21. Unit-тесты сервера
  ...
  - _Зависит от: задача 3, 4_
  - _55 тестов: `server/test/`._

- [x] 22. Unit-тесты клиента
  ...
  - _Зависит от: задача 10, 12_
  - _29 тестов: Vitest в `client/src/`._

- [x] 23. Integration-тесты сокет-слоя
  ...
  - _Реализация: `server/test/socket.integration.test.js`._

- [x] 24. E2E-тесты (Playwright, fake media)
  ...
  - _8 сценариев: `e2e/video-chat.spec.js`._

---

## Блок H. Деплой и документация

- [x] 25. CI/CD + контейнеризация
  ...
  - _Реализация: `.github/workflows/ci.yml`, `Dockerfile`, `docker-compose.yml`, `deploy/nginx.conf`._

- [x] 26. README и эксплуатационная документация
...
1. **Поведение при недостижимом peer (строгий NAT)** — влияет на задачу 20. _(TDD §14, п.1)_ — **решено частично:** индикатор на плитке, без TURN; участник остаётся в комнате.
2. **Значения rate-limit / длины сообщения** — влияет на задачи 4, 8. _(TDD §14, п.2)_ — **решено:** `MESSAGE_MAX_LEN=1000`; rate-limit на сокет не реализован.
3. **ICE restart при кратковременном сбое** — влияет на задачу 10. По умолчанию не реализуем. _(TDD §14, п.3)_ — **не реализуем** (PRD F-18).
4. **Нативный `RTCPeerConnection` vs `simple-peer`** — влияет на задачу 10. Рекомендация — нативный. _(TDD §14, п.4)_ — **решено:** нативный API.
5. **`nanoid` vs `UUID` для `roomId`** — влияет на задачи 1, 13. Рекомендация — `nanoid(8)`. _(TDD §14, п.5)_ — **решено:** `nanoid(8)`.

---

## Трассировка PRD → реализация (кратко)

| PRD | Реализация |
|-----|------------|
| F-01…F-05 | `StartScreen`, `RoomScreen`, `rooms.js`, `roomHandlers.js` |
| F-06…F-10 | `useLocalMedia`, `PeerConnectionManager`, `Controls`, `VideoTile` |
| F-12…F-15 | `chat.js`, `ChatPanel` |
